...
Dealing with missing value is not so trivial. The easiest case to deal with missing value would be to treat them as missing at random and drop them. However, it is hard to prove that data are missing at random. Given the nature of this data set, my intuition is that people did their due dilligence while collecting this data since houses are significant assets in average households.
...
The limitation of best subset selection is that when the number of variables p becomes very large, the computation becomes infeasible. I believe the rule of thumb is more than 40 variables, but we have 78 meaning the computer has to calculate 2^78 possible models!

```{r, error = TRUE}
regfit.full <- regsubsets(SalePrice ~ ., train)
```

Fortunately, there is an alternative option of stepwise selection. The number of models that need to be calculated is pared down to `r 78(78 + 1)/2 + 1`. 

### Forward Stepwise Selection

Forward stepwise selection starts with a model with no regressors and subsequently add one variable that results in the greatest improvement to the fit.
...
We get a widely varying number of features depending on metrics we use.
...
df2 <- df[,!names(df) %in% drop.cols]
dim(df2)
train <- df[1:1460,]
test <- df[1461:2919,]
pcr.fit <- pcr(SalePrice ~., data = train, scale = FALSE, validation = "CV")
```
